...
void cppPNML::details::ddTransition::write_pnml(pugi::xml_node& xnode) const {
  xnode.set_name("transition");
...
bool cppPNML::details::ddTransition::read_pnml(const pugi::xml_node& xnode, ddObj *pf) {
  ...
  if(!static_cast<ddGraph *>(pf)
     ->add(
           static_pointer_cast<ddTransition>
           (shared_from_this())
           ) 
     )return false;
...
void cppPNML::details::ddTransition::write_ogdf(ogdf::Graph *pg, ogdf::GraphAttributes *pga,
  ...
  pga->shapeNode(pnode) = ogdf::GraphAttributes::rectangle;
}

void cppPNML::details::ddTransition::read_ogdf(void *p, ogdf::GraphAttributes *pga) {
