...
void SDFG::dfgGraph::add_path(shared_ptr<dfgPath> p) {
  ...
  if(p->path.empty()) {         // path generated by fast algorithm
    if(exist(divide_signal_name(p->src->get_hier_name()))) {
      s = get_node(divide_signal_name(p->src->get_hier_name()));
    } else {
      s.reset(p->src->copy());
      add_node(s);
    }
    if(exist(divide_signal_name(p->tar->get_hier_name()))) {
      t = get_node(divide_signal_name(p->tar->get_hier_name()));
    } else {
      t.reset(p->tar->copy());
      add_node(t);
    }
    ...
    while(!m_path.empty()) {
      if(exist(divide_signal_name(m_path.front().first->get_hier_name()))) {
        s = get_node(divide_signal_name(m_path.front().first->get_hier_name()));
      } else {
      ...
      if(exist(divide_signal_name(tar->get_hier_name()))) {
        t = get_node(divide_signal_name(tar->get_hier_name()));
      } else {
        t.reset(tar->copy());
        add_node(t);
      }
...
void SDFG::dfgGraph::remove_node(vertex_descriptor nid) {
  ...
  // remove the node
  pair<string, dfgRange> name_range = divide_signal_name(pn->get_hier_name());
  node_map[name_range.first].erase(pn);
  nodes.erase(nid);
             ...
             if((m.second->type & dfgNode::SDFG_IPORT) == dfgNode::SDFG_IPORT) {
               ...
               if(m.second->type == dfgNode::SDFG_PORT) {
                 ...
                 if(father->port2sig.count(m.second->get_hier_name())) {
                   ...
                   shared_ptr<dfgNode> snode = father->pg->get_node(divide_signal_name(sname));
...
std::set<shared_ptr<dfgNode> > SDFG::dfgGraph::get_node(const string& nname) const {
  pair<string, dfgRange> name_range = divide_signal_name(nname);
...
shared_ptr<dfgNode> SDFG::dfgGraph::get_node(pair<string, dfgRange> name_range) const {
  if(!node_map.count(name_range.first)) return shared_ptr<dfgNode>();
  ...
  if(inode->type == dfgNode::SDFG_MODULE && inode->child) {
    ...
    if(!plist.empty()) {
      BOOST_FOREACH(const string& m, plist) {
        shared_ptr<dfgNode> n = inode->child->get_node(divide_signal_name(m));
...
list<shared_ptr<dfgNode> > SDFG::dfgGraph::get_target_cb(edge_descriptor eid) const {
  ...
  if(onode->type == dfgNode::SDFG_MODULE && onode->child) {
    ...
    if(!plist.empty()) {
      BOOST_FOREACH(const string& m, plist) {
        shared_ptr<dfgNode> n = onode->child->get_node(divide_signal_name(m));
...
// hierarchical search
///////////////////////////////
std::set<shared_ptr<dfgNode> > SDFG::dfgGraph::search_node(const string& nname) const {
  ...
  if(exist(dummy->get_hier_name())) // check node name
    ...
    if(exist(divide_signal_name(gname)) && get_node(divide_signal_name(gname))->type == dfgNode::SDFG_MODULE && get_node(divide_signal_name(gname))->child)
      return get_node(divide_signal_name(gname))->child->search_node(dummy->get_hier_name());
    else
      return std::set<shared_ptr<dfgNode> >(); // failed, return empty
  } else
    return std::set<shared_ptr<dfgNode> >(); // failed, return empty
}
  ...
  BOOST_FOREACH(shared_ptr<dfgNode> node, nlook_list) {
    if(node->type & dfgNode::SDFG_MODULE) {
      if(hier && node->child) {
        ...
        if(node->child->nodes.size() == 0)
          remove_node(node);
      }
    } else if(!nlist.count(node))
      remove_node(node);
  }
}
...
bool SDFG::dfgGraph::exist(const string& nname) const {
  return get_node(nname).size();
}    

bool SDFG::dfgGraph::exist(pair<string, dfgRange> name_range) const {
  return get_node(name_range);
}    

bool SDFG::dfgGraph::exist(shared_ptr<dfgNode> n) const {
  return nodes.count(n->id);
}    
  ...
  if(pn->type == dfgNode::SDFG_OPORT || pn->type == dfgNode::SDFG_PORT) { // output or I/O port
    if(father && father->port2sig.count(pn->get_hier_name())) {
      if(father->port2sig.find(pn->get_hier_name())->second.size() > 0)
        rv.push_back(father->pg->get_node(divide_signal_name(father->port2sig.find(pn->get_hier_name())->second)));
    }
  } 
    ...
    for(boost::tie(nit, nend) = boost::adjacent_vertices(nid, bg_);
      ...
      if(osrc->type == dfgNode::SDFG_MODULE) { // it is a module entity, go further
        BOOST_FOREACH(const string& sname, osrc->sig2port.find(pn->name)->second) {
          rv.push_back(osrc->child->get_node(divide_signal_name(sname)));
        }
    ...
    for(boost::tie(nit, nend) = boost::inv_adjacent_vertices(nid, bg_);
      ...
      if(isrc->type == dfgNode::SDFG_MODULE) { // it is a module entity, go further
        BOOST_FOREACH(const string& sname, isrc->sig2port.find(pn->name)->second) {
          rv.push_back(isrc->child->get_node(divide_signal_name(sname)));
        }
      ...
      if(father && father->port2sig.count(pn->get_hier_name())) {
        ...
        if(tar_name.size() > 0) {
          ...
          for(boost::tie(eit, eend) = 
                boost::edge_range(
                                  father->id, 
                                  father->pg->get_node(divide_signal_name(tar_name))->id, 
                                  father->pg->bg_);
              eit != eend; ++eit) { 
...
bool SDFG::dfgGraph::check_integrity() const {
  ...
  BOOST_FOREACH(node_map_type nm, node_map) {
    BOOST_FOREACH(shared_ptr<dfgNode> mm, nm.second) {
      assert(mm);
      vertex_descriptor nid = mm->id;
      assert(nodes.count(nid));
      assert(nodes.find(nid)->second == mm);
    }
  }
  ...
  BOOST_FOREACH(nodes_type n, nodes) {
    ...
    assert(get_node(divide_signal_name(pn->get_hier_name())));
    assert(pn->pg == this);
